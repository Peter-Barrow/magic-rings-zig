...
pub fn MakeConfig(comptime project_name: []const u8, comptime ElementType: type) type {
    return struct {
        const Self = @This();
        ...
        project_name: []const u8 = project_name,
        name: []const u8,
        shm_path: []const u8,
        num_connections: usize,
        library_version: []const u8 = "1.0.0", // Replace "1.0.0" with the actual version of your library
        shm_size: usize,

        // Comptime fields derived from ElementType
        const element_size: comptime_int = @sizeOf(ElementType);
        const element_type: []const u8 = @typeName(ElementType);
        ...
        fn makeFilePath(allocator: std.mem.Allocator, name: []const u8) ![]const u8 {
            ...
            const app_config_dir = try std.fs.path.join(
                allocator,
                &[_][]const u8{ config_dir, project_name },
            );
            defer allocator.free(app_config_dir);

            const file_name = try std.fmt.allocPrint(allocator, "{s}_config.json", .{name});
        ...
        pub fn writeConfig(self: *const Self, allocator: std.mem.Allocator) !void {
            const file_path = try Self.makeFilePath(allocator, self.name);
        ...
        pub fn loadConfig(allocator: std.mem.Allocator, name: []const u8) !Self {
            const file_path = try Self.makeFilePath(allocator, name);
            defer allocator.free(file_path);
...
test "getConfigFilePath" {
    const TestConfig = MakeConfig("test_project", u32);
    const allocator = std.testing.allocator;

    const test_config = TestConfig.init("test_config", "/tmp/test_shm", 1024);
    const file_path = try TestConfig.makeFilePath(allocator, test_config.name);
    defer allocator.free(file_path);
